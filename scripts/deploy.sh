...
if [ -z "$TRAVIS_PULL_REQUEST" ]; then
    ...
    if [ "$TRAVIS_BRANCH" = "master" ]; then
        if [ "$TRAVIS_PHP_VERSION" = "7.0.27" ]; then
            ...
            sshpass -e ssh -o stricthostkeychecking=no travis@ana.decock.org "cd /vahi/deploy/api/$TRAVIS_BRANCH/builds ; tar -xzf vahi.tgz ; rm vahi.tgz ; rm -rf previous ; mv current previous ; mv build current ; cd current/public ; ln -s /vahi/storage/api/master/i ; ln -s /vahi/storage/api/master/export"
            echo "All done.";
        else
